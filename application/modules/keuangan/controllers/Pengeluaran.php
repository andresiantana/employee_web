...
class Pengeluaran extends CI_Controller {
	...
	public function index($id = NULL)
	{
		...
		if($nip != '' || $nama != '' || $kode_pengeluaran != '' ||  $tanggal_awal != '' || $tanggal_akhir != ''){
			$data['data']	= $this->KUPencairanBiayaT->tampilLaporanPengeluaran($config['per_page'],$id,$nip,$nama,$kode_pengeluaran,$tanggal_awal,$tanggal_akhir)->result_object();
			$tr['tr'] = $this->load->view('keuangan/pengeluaran/pencarian',$data,true);
			echo json_encode($tr['tr']); 
			exit;
		}else{
			$data['data']	= $this->KUPencairanBiayaT->tampilLaporanPengeluaran($config['per_page'],$id)->result_object();
		}	
	...
	public function detail($id_pencairan_biaya = null,$id_pegawai=null) {
		...
		$tr['tr'] = $this->load->view('keuangan/pengeluaran/detail',$data,true);
		echo json_encode($tr['tr']); 
		exit;
	}
	...
	public function printDetailPengeluaran($id_pengeluaran_biaya = null, $id_pegawai = null) {
		...
		$data['data_row'] = $this->db->select('pengajuan_biaya.id_pegawai,pengajuan_biaya.tanggal,pengajuan_biaya.kode_pengajuan,pengajuan_biaya.id_pencairan_biaya,pengajuan_biaya.semester,
							...
							->from('pengajuan_biaya')
							->join('pencairan_biaya', 'pencairan_biaya.id_pencairan_biaya = pengajuan_biaya.id_pencairan_biaya','left')
							->where('pengajuan_biaya.id_pencairan_biaya',$id_pengeluaran_biaya)
							->get()->row();
		$data['datapegawai'] = $this->db->get_where('pegawai',array('id_pegawai'=>$id_pegawai))->row();
		$this->load->view('keuangan/pengeluaran/print', $data);
	}
}
